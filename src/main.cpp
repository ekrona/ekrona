...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ekrona: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ekrona Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    // Subsidy is cut in half every 500000  blocks, which will occur approximately every 8 month
...
static const int64 nTargetTimespan = 18 * 60 * 60; // ekrona: 18  hours
static const int64 nTargetSpacing = 200; // ekrona: 3.3 minutes

static const int64 nTargetTimespan_shield = 200; // ekrona: 3.3 minutes
static const int64 nTargetSpacing_shield = 200; // ekrona: 3.3 minutes
static const int64 nInterval_shield = nTargetTimespan_shield / nTargetSpacing_shield; // Each block
...
unsigned int static GetNextWorkRequired_Original(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // ekrona difficulty adjustment protocol switch
    int64 nDiffSwitchBlock = 1000;
    int nHeight = pindexLast->nHeight + 1;
    bool fNewDifficultyProtocol = (nHeight >= nDiffSwitchBlock || fTestNet);

    int64 nTargetTimespanCurrent = fNewDifficultyProtocol? nTargetTimespan : (7 * 18 * 60 * 60);
    ...
    // ekrona: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired_DigiShield(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    int64 retargetTimespan = nTargetTimespan_shield;
    int64 retargetSpacing = nTargetSpacing_shield;
    int64 retargetInterval = nInterval_shield;
    // Genesis block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
		...
		if (height >= 45000) { DiffMode = 2; }

        if             (DiffMode == 1) { return GetNextWorkRequired_Original(pindexLast, pblock); } //legacy diff mode
        else if        (DiffMode == 2) { return GetNextWorkRequired_DigiShield(pindexLast, pblock); } // DigiShield
        return GetNextWorkRequired_DigiShield(pindexLast, pblock); // DigiShield
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal ekrona miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ekrona", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "ekrona - for world with the times - January 2014, 5 years after Bitcoin.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb92a6827bf6fe1c7b6c8e2929e753836065652440df4970edbf706756a37787f"));
...
unsigned char pchMessageStart[4] = { 0xdc, 0xc1, 0xc1, 0x04 };
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // ekronad: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
